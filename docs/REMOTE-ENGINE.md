...
Внутренний API движка (`8081`) сейчас **без аутентификации**. Обязательно ограничь доступ к нему
файрволом, разрешив только IP оркестратора. Шифрованный транспорт (mTLS) — в планах (Фаза 4.5).
