...
**Анонсы к трекерам — раздельно по движкам.** Каждый движок — отдельная libtorrent-сессия со
своим **BitTorrent listen-портом** (b1→50001 … b6→50006, проброшены на хост tcp+udp) и своим
`peer_id`/DHT. Поэтому к трекеру каждый движок анонсится **сам за себя**, со своим портом —
с точки зрения трекера это независимые BitTorrent-клиенты. Никакого общего анонсера нет.
Текущий порт виден в `GET /api/v1/session/stats` → `by_engine.<id>.listening_port`.
...
**Бэкап** (подробно — [`docs/BACKUP.md`](docs/BACKUP.md)): cron на CT400 в 04:30 снимает
дамп Postgres и состояние каждого движка (`.fastresume`/`.torrents`/`.state`) через
`GET /internal/v1/meta/archive`, кладёт в `/mnt/media/seeding-test/_backups/db` (он же
`/backups` в контейнере `api`) с ретеншеном 14 дней. Контент раздач не копируется.
Откат БД — кнопкой в настройках; состояние движка — процедурой на хосте.
...
**Состояние для списка/детали:** список читает снимок БД (~10 с). Деталь берёт
runtime у движка (имя, размер, прогресс, ↓/↑, отдано, ratio, ETA, added, лимиты,
файлы, трекеры, список подключённых пиров) и объединяет с БД.

**Сиды и пиры в таблице — рой трекера, не коннекты.** libtorrent `num_seeds` считает
только чужих сидов в соединениях, а анонс RuDub не кладёт `complete` в ответ, поэтому
эти поля были нулём. Движок ходит на `scrape.php` и пишет `complete`/`incomplete` в
`torrents.seeds` и `torrents.leechers` (NULL, пока скрейпа не было). Колонки «Сиды» и
«Пиры» показывают их. `torrents.peers` по-прежнему число коннектов: фильтр «есть пиры»
и список пиров на странице раздачи. Скрейп идёт фоном и не блокирует
`GET /session/stats`: от этого ответа зависит список движков в «Перенести на…».

**«Всего отдано» — накопитель в БД, а не показание движка.** Счётчик libtorrent живёт в
рамках одной инкарнации торрента: перенос на другой движок добавляет раздачу заново, и
счётчик там стартует с нуля. Объём отданного принадлежит раздаче, поэтому фоновый снимок
(`runtime_snapshot`) прибавляет к `torrents.uploaded_total` дельту, а `torrents.uploaded_seen`
хранит последнее сырое показание текущего движка (инвариант: `uploaded_total - uploaded_seen`
= отдано на прошлых движках). Уход счётчика назад = новая инкарнация; при переносе база
дельт обнуляется явно в `update_engine`. В ответы API значение подставляет
`runtime_sync.apply_uploaded_carry` — «накопленное + живое», чтобы цифра была верной сразу,
...
**Самовосстановление движка (engine self-restore).** Движок при старте (`start()`,
флаг `SEEDING_ENGINE_SELF_RESTORE`, по умолчанию вкл.) сам поднимает все раздачи со своего
тома — из `.fastresume` (а где нет — из `.torrents/<db_id>.torrent`). Это значит, что
**перезапуск только движков не требует участия API**: контейнер движка вернулся → раздачи
снова сидируются. Том движка зеркалит назначенные ему раздачи (файл создаётся при add,
удаляется при remove), поэтому restore с диска эквивалентен состоянию БД для этого движка.
API-restore (ниже) остаётся как восстановление по источнику правды (БД) — например, для
свежего движка с пустым томом.
